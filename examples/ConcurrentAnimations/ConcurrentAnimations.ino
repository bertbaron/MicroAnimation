 ...
 * code as long as the loop is fast enough. The maximum achievable framerate may
 * be impacted though because of the required computation time, depending on the
 * hardware and the complexity of the animations.
 * 
...
void printCentered(String text, int y) { printCentered(text, SCREEN_WIDTH / 2, y); }
...
void printCentered(String text, int xCenter, int y) {
  ...
  x = min(x, SCREEN_WIDTH - (int)textWidth);
  x = max(x, 0);
